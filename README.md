# Hi, I'm Suhas

Software engineering student . I build, test, and learn.

## About me

* Focus areas: programming, problem solving, systems basics
* Interested in backend, performance, and tooling
* Learning by building small, clear projects

## Skills

* Python ![Python](https://img.shields.io/badge/Python-3776AB?logo=python\&logoColor=white)
* C++ ![C++](https://img.shields.io/badge/C%2B%2B-00599C?logo=c%2B%2B\&logoColor=white)
* Node.js ![Node.js](https://img.shields.io/badge/Node.js-339933?logo=node.js\&logoColor=white)
* HTML ![HTML](https://img.shields.io/badge/HTML5-E34F26?logo=html5\&logoColor=white)
* CSS ![CSS](https://img.shields.io/badge/CSS3-1572B6?logo=css3\&logoColor=white)
* GitHub ![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github\&logoColor=white)

## What you will find here

* Practice projects for coursework
* Algorithm and data structure implementations
* Experiments with tools and platforms

## Current goals

* Write clean, readable code
* Improve time and space analysis
* Build one complete project end to end

## How I work

* Small commits
* Clear names
* Simple solutions first

## Contact

* GitHub: @Suhas-Saur
* Email: [suhassuperstar@gmail.com](mailto:suhassuperstar@gmail.com)

Thanks for stopping by.
